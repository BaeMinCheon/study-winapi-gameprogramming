	...
	_In_ int p_nShow)
{
	...
	HWND hWnd = NULL;
	MSG* pMsg = NULL;
	TCHAR* pszApplicationName = TEXT("00 First");
...
HRESULT __stdcall OnPaint(_In_ HWND p_hWnd)
{
	...
	static const UINT iTestCount = 2000;
	...
	for (int x = 0; x < 100; ++x)
	{
		Rectangle(hCom, 50, 50, 750, 550);
	}

	BYTE* pMem = NULL;
	pMem = reinterpret_cast<BYTE*>(malloc(800 * 600 * 4));
...
#pragma omp parallel for
		...
		{
			memset(pMem + 50 * 4 + 800 * 4 * y, 255, 700 * 4);
		}
	}
